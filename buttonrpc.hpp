...
void __package_params_impl(Serializer &ds, const Tuple &t, std::index_sequence<Is...>) {
...
void package_params(Serializer &ds, const std::tuple<Args...> &t) {
    ...
    __package_params_impl(ds, t, std::index_sequence_for<Args...>{});
}
...
decltype(auto) invoke(Function &&func, Tuple &&t) {
    constexpr auto t_size = std::tuple_size<typename std::decay<Tuple>::type>::value;
    return invoke_impl(std::forward<Function>(func), std::forward<Tuple>(t), std::make_index_sequence<t_size>{});
}
...
inline void buttonrpc::bind(std::string name, F func) {
    m_handlers[name] = std::bind(&buttonrpc::callproxy<F>, this, func, std::placeholders::_1, std::placeholders::_2,
                                 std::placeholders::_3);
}

template<typename F, typename S>
inline void buttonrpc::bind(std::string name, F func, S *s) {
    m_handlers[name] = std::bind(&buttonrpc::callproxy<F, S>, this, func, s, std::placeholders::_1,
